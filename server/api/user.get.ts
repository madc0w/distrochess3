import { createError, defineEventHandler } from 'h3';
import type { UserDoc } from '../types/user';
import { verifyAuthToken } from '../utils/auth';
import { getDb } from '../utils/mongo';

export default defineEventHandler(async (event) => {
	const userId = verifyAuthToken(event);
	const db = await getDb();
	const user = await db.collection<UserDoc>('users').findOne(
		{ _id: userId },
		{
			projection: {
				name: 1,
				email: 1,
				score: 1,
				wins: 1,
				losses: 1,
				draws: 1,
				createdDate: 1,
				preferredLocale: 1,
				duckOpinion: 1,
				popitScore: 1,
				popitTrials: 1,
				picapicScore: 1,
				picapicTrials: 1,
			},
		}
	);

	if (!user) {
		throw createError({
			statusCode: 404,
			statusMessage: 'ERR_USER_NOT_FOUND',
		});
	}

	return {
		_id: userId.toString(),
		name: user.name,
		email: user.email,
		score: user.score ?? 0,
		wins: user.wins?.map((id) => id.toString()) ?? [],
		losses: user.losses?.map((id) => id.toString()) ?? [],
		draws: user.draws?.map((id) => id.toString()) ?? [],
		preferredLocale: user.preferredLocale,
		createdDate: user.createdDate?.toISOString() ?? null,
		duckOpinion: user.duckOpinion,
		popitScore: user.popitScore ?? 400,
		popitTrials: user.popitTrials ?? 0,
		picapicScore: user.picapicScore ?? 400,
		picapicTrials: user.picapicTrials ?? 0,
	};
});
